...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    foreach (['category_id','brand','model','purchase_year','condition_grade','description','sale_price','rental_price_day','reuse_count','distance_km','city'] as $key) $values[$key] = trim((string)($_POST[$key] ?? ''));
    ...
    if(!$errors){
        ...
        try{
            ...
            $update=$pdo->prepare('UPDATE equipment SET category_id=:category,brand=:brand,model=:model,purchase_year=:year,condition_grade=:condition,description=:description,sale_price=:sale,rental_price_day=:rental,available_for_sale=:for_sale,available_for_rental=:for_rental,circular_score=:score,reuse_count=:reuse,repaired=:repaired,distance_km=:distance,city=:city,service_discount=:discount WHERE id=:id AND owner_id=:owner');
            $update->execute(['category'=>$categoryId,'brand'=>$values['brand'],'model'=>$values['model'],'year'=>$year,'condition'=>$values['condition_grade'],'description'=>$values['description'],'sale'=>$values['available_for_sale']?(float)$sale:null,'rental'=>$values['available_for_rental']?(float)$rental:null,'for_sale'=>(int)$values['available_for_sale'],'for_rental'=>(int)$values['available_for_rental'],'score'=>$score,'reuse'=>$reuse,'repaired'=>(int)$values['repaired'],'distance'=>$distance,'city'=>$values['city'],'discount'=>$tier['discount'],'id'=>$id,'owner'=>$user['id']]);
