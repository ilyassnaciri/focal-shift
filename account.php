...
$pdo = db();
$offers = [];
$statement = $pdo->prepare('SELECT id,brand,model,status,circular_score FROM equipment WHERE owner_id=:id ORDER BY created_at DESC');
...
<main id="main-content">
    ...
    <section class="section"><div class="container account-grid">
        <article class="account-card"><span class="account-number"><?= count($offers) ?></span><h2>Mes offres</h2><p>Équipements publiés en vente ou location.</p><?php if ($offers): ?><ul class="account-list"><?php foreach ($offers as $offer): $offerUrl=$offer['status']==='pending_verification'?'verification.php?id='.(int)$offer['id']:'product.php?id='.(int)$offer['id']; ?><li><a href="<?= url($offerUrl) ?>"><?= e($offer['brand'] . ' ' . $offer['model']) ?></a><span class="offer-list-meta"><b class="offer-status offer-status-<?= e($offer['status']) ?>"><?= $offer['status']==='reserved'?'Réservé / loué':($offer['status']==='published'?'Disponible':($offer['status']==='pending_verification'?'Vérification requise':e($offer['status']))) ?></b></span></li><?php endforeach; ?></ul><?php endif; ?><a class="button button-secondary" href="<?= url('deposit.php') ?>">Publier une offre</a></article>
